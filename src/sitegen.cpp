...
void Sitegen::make_diagram(uint32_t tileref)
{
...
void Sitegen::mark_districts(void)
{
...
void Sitegen::mark_junctions(void)
{
	std::unordered_map<int, bool> visited;
...
void Sitegen::outline_walls(size_t radius)
{
...
void Sitegen::finalize_walls(void)
{
	std::map<std::pair<int, int>, bool> link; // two connected cells
	for (auto &sect : sections) {
	...
	for (auto &wall : walls) { polygon.push_back(wall.S.P0); }
	if (clockwise_polygon(polygon)) {
		for (auto &wall : walls) {
			std::swap(wall.S.P0, wall.S.P1);
			std::swap(wall.left, wall.right);
		}
	}
}

void Sitegen::make_highways(void)
{
...
void Sitegen::divide_parcels(size_t radius)
{
	for (auto &d : districts) {
